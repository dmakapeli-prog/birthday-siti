  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-6"
      ...
      <div className="mb-4" style={{ transition: 'opacity 1.5s ease', opacity: show1 ? 1 : 0 }}>
        <span className="font-cormorant italic" style={{ fontSize: 'clamp(3.5rem,9vw,6.5rem)', fontWeight: 700, color: 'var(--ink)' }}>Happy Birthday</span>
      </div>
      <div className="mb-3" style={{ transition: 'opacity 1.5s ease', opacity: show2 ? 1 : 0 }}>
        <span className="font-cormorant italic" style={{ fontSize: 'clamp(2.2rem,5vw,4rem)', color: 'var(--rose)', fontWeight: 400 }}>With All My Love</span>
      </div>
      <div className="mb-8" style={{ transition: 'opacity 1.5s ease', opacity: show3 ? 1 : 0 }}>
        <span className="font-cormorant italic" style={{ fontSize: 'clamp(1.8rem,4vw,3rem)', color: 'var(--ink)', fontWeight: 300 }}>Always &amp; Forever</span>
      </div>
      ...
      <div style={{ transition: 'opacity 1.5s ease', opacity: showSub ? 1 : 0 }}>
        ...
        <p className="font-dancing mb-8" style={{ fontSize: 'clamp(1.8rem,3.5vw,2.5rem)', color: 'var(--rose)' }}>
          Forever yours. 🤍
        </p>
        <button onClick={onReplay} className="px-8 py-3 rounded-full text-sm tracking-[3px] uppercase transition-all hover:scale-105" style={{ border: '1px solid var(--rose)', color: 'var(--ink)', background: 'rgba(255,255,255,0.5)', fontFamily: 'sans-serif' }}>
          ↺ REPLAY FROM START
        </button>
      </div>
    </div>
