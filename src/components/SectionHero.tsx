    ...
    <div className="relative flex flex-col items-center justify-center h-full text-center px-4 sm:px-6 overflow-hidden"
      ...
      {polaroids.map((p, i) => (
        <motion.div
          ...
          className="absolute transition-transform duration-500 hover:scale-110 hover:z-20 cursor-pointer hidden sm:block"
      ...
      {/* Center content */}
      <motion.div
        className="relative z-10 max-w-xl px-2 flex flex-col items-center"
        variants={containerVariants}
        initial="hidden"
        animate="visible"
      >
        <motion.div variants={itemVariants} className="text-xs tracking-[4px] sm:tracking-[6px] uppercase mb-2 sm:mb-5" style={{ color: 'var(--rose)', fontFamily: 'sans-serif' }}>
          ✦ A LOVE LETTER FOR YOU ✦
        </motion.div>
        <motion.h2 variants={itemVariants} className="font-cormorant italic font-semibold mb-2 sm:mb-4" style={{ fontSize: 'clamp(2.8rem,10vw,7.5rem)', lineHeight: 0.95, textShadow: '0 0 40px rgba(232,130,159,0.4), 0 4px 20px rgba(232,130,159,0.15)', color: 'var(--ink)' }}>
          Your<br className="hidden sm:inline" /> Special Day
        </motion.h2>

        {/* Mobile Polaroid Grid */}
        <motion.div variants={itemVariants} className="grid grid-cols-2 gap-2.5 w-full max-w-[260px] my-2 sm:hidden">
          {polaroids.map((p, i) => {
